...
GitHub Pages at `https://fahmidhasann.github.io/` (also configured for Vercel — see `.vercel` in `.gitignore`). Keep `<link rel="canonical">` in `index.html` in sync if the domain changes.
